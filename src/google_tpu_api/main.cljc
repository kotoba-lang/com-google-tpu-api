(ns google-tpu-api.main
  "Kotodama WASM entrypoint for the Google_tpu_api clean-room actor (L4) — Clojure port.
  ...
  facts (`google_tpu_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "google_tpu_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"        :plural "devices"        :id-prefix "googletp_dev"
    :fields [:name :vendor :memoryGb :index]         :required [:name :vendor]
    :coerce {:memoryGb :int :index :int}             :refs {}}
   {:entity "Kernel"        :plural "kernels"        :id-prefix "googletp_ker"
    :fields [:deviceId :name :gridDim :blockDim]     :required [:name :gridDim]
    :coerce {}                                        :refs {:deviceId "Device"}}
   {:entity "MemoryAlloc"   :plural "memoryallocs"   :id-prefix "googletp_mem"
    :fields [:deviceId :sizeBytes :ptr]              :required [:sizeBytes :ptr]
    :coerce {:sizeBytes :int}                        :refs {:deviceId "Device"}}
   {:entity "InferenceJob"  :plural "inferencejobs"  :id-prefix "googletp_inf"
    :fields [:modelRef :deviceId :status :latencyMs] :required [:status :latencyMs]
    :coerce {:latencyMs :float}                      :refs {:deviceId "Device"}}
   {:entity "Engine"        :plural "engines"        :id-prefix "googletp_eng"
    :fields [:name :precision :maxBatch]             :required [:name :precision]
    :coerce {:maxBatch :int}                         :refs {}}
   {:entity "Utilization"   :plural "utilizations"   :id-prefix "googletp_uti"
    :fields [:deviceId :gpuPct :memPct :sampledAt]   :required [:gpuPct :memPct]
    :coerce {:gpuPct :float :memPct :float}          :refs {:deviceId "Device"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"google_tpu_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "google_tpu_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :google_tpu_api-compat/ready)
