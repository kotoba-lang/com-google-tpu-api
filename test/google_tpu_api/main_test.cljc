(ns google-tpu-api.main-test
  "Contract + behavioral test for the google-tpu-api L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "google_tpu_api-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'google-tpu-api.main-test)]
